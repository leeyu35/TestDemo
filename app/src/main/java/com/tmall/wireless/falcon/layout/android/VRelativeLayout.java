...
import static com.libra.common.StringTab.STR_ID_VRELATIVE_LAYOUT_ATTR_BASE;

/**
 * Created by gujicheng on 17/6/5.
 */

public class VRelativeLayout extends VLayoutBase {
    private final static String TAG = "VRelativeLayout";
    ...
    private IView mBaselineView = null;
    ...
    private SortedSet<IView> mTopToBottomLeftToRightSet = null;

    private boolean mDirtyHierarchy = true;
    private IView[] mSortedHorizontalChildren;
    private IView[] mSortedVerticalChildren;
    ...
    public static class Builder implements IBuilder {
        ...
        public IView build(PageContext pageContext, ViewCache viewCache) {
            return new VRelativeLayout(pageContext, viewCache);
        }
    }

    public VRelativeLayout(PageContext pageContext, ViewCache viewCache) {
...
//        mAllowBrokenMeasureSpecs = version <= JELLY_BEAN_MR1;
...
//
    private void sortChildren() {
        ...
        if (mSortedVerticalChildren == null || mSortedVerticalChildren.length != count) {
            mSortedVerticalChildren = new IView[count];
        }

        if (mSortedHorizontalChildren == null || mSortedHorizontalChildren.length != count) {
            mSortedHorizontalChildren = new IView[count];
        }
...
//        }
        ...
        IView[] views = mSortedHorizontalChildren;
        ...
        final int targetSdkVersion = mPageContext.getContext().getApplicationInfo().targetSdkVersion;

        for (int i = 0; i < count; i++) {
            final IView child = views[i];
            if (!child.isGone()) {
                final LayoutParams params = (LayoutParams) child.getVLayoutParams();

                applyVerticalSizeRules(params, myHeight, -1);
        ...
        IView baselineView = null;
        LayoutParams baselineParams = null;
        for (int i = 0; i < count; i++) {
            final IView child = views[i];
            if (!child.isGone()) {
                final LayoutParams childParams = (LayoutParams) child.getVLayoutParams();
...
//            height = Math.max(height, getSuggestedMinimumHeight());
        ...
        setVMeasuredDimension(width, height);
    }
     ...
     */
    private void measureChild(IView child, LayoutParams params, int myWidth, int myHeight) {
    ...
    private void measureChildHorizontal(
            IView child, LayoutParams params, int myWidth, int myHeight) {
    ...
    private int getChildMeasureSpec(int childStart, int childEnd,
        ...
        int childSpecMode = View.MeasureSpec.UNSPECIFIED;
        int childSpecSize = 0;
        ...
        if (childStart != VALUE_NOT_SET && childEnd != VALUE_NOT_SET) {
            ...
            childSpecMode = isUnspecified ? View.MeasureSpec.UNSPECIFIED : View.MeasureSpec.EXACTLY;
    ...
    private boolean positionChildHorizontal(IView child, LayoutParams params, int myWidth,
                                            boolean wrapContent) {
...
//        final int layoutDirection = getLayoutDirection();
        ...
        if (params.mLeft == VALUE_NOT_SET && params.mRight != VALUE_NOT_SET) {
            ...
            params.mLeft = params.mRight - child.getVMeasuredWidth();
        } else if (params.mLeft != VALUE_NOT_SET && params.mRight == VALUE_NOT_SET) {
            // Left is fixed, but right varies
            params.mRight = params.mLeft + child.getVMeasuredWidth();
        } else if (params.mLeft == VALUE_NOT_SET && params.mRight == VALUE_NOT_SET) {
...
//                }
    ...
    private boolean positionChildVertical(IView child, LayoutParams params, int myHeight,
                                          boolean wrapContent) {
        ...
        if (params.mTop == VALUE_NOT_SET && params.mBottom != VALUE_NOT_SET) {
            ...
            params.mTop = params.mBottom - child.getVMeasuredHeight();
        } else if (params.mTop != VALUE_NOT_SET && params.mBottom == VALUE_NOT_SET) {
            // Top is fixed, but bottom varies
            params.mBottom = params.mTop + child.getVMeasuredHeight();
        } else if (params.mTop == VALUE_NOT_SET && params.mBottom == VALUE_NOT_SET) {
    ...
    private IView getRelatedView(int[] rules, int relation) {
        ...
        if (id != 0) {
            ...
            while (v.isGone()) {
//                rules = ((LayoutParams) v.getVLayoutParams()).getRules(v.getLayoutDirection());
                rules = ((LayoutParams) v.getVLayoutParams()).getRules(0);
    ...
    private LayoutParams getRelatedViewParams(int[] rules, int relation) {
        IView v = getRelatedView(rules, relation);
        if (v != null) {
            ViewGroup.LayoutParams params = v.getVLayoutParams();
            if (params instanceof LayoutParams) {
                return (LayoutParams) v.getVLayoutParams();
            }
        }
        return null;
    }
    ...
    private int getRelatedViewBaselineOffset(int[] rules) {
        final IView v = getRelatedView(rules, ALIGN_BASELINE);
        if (v != null) {
            final int baseline = -1;
            if (baseline != -1) {
                final ViewGroup.LayoutParams params = v.getVLayoutParams();
                if (params instanceof LayoutParams) {
                    final LayoutParams anchorParams = (LayoutParams) v.getVLayoutParams();
    ...
    private static void centerHorizontal(IView child, LayoutParams params, int myWidth) {
        int childWidth = child.getVMeasuredWidth();
    ...
    private static void centerVertical(IView child, LayoutParams params, int myHeight) {
        int childHeight = child.getVMeasuredHeight();
    ...
    public void onVLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (!child.isGone()) {
                LayoutParams st =
                        (LayoutParams) child.getVLayoutParams();
                child.vLayout(st.mLeft, st.mTop, st.mRight, st.mBottom);
            }
        }
    }
...
//    }
    ...
    @Override
    public VRelativeLayout.LayoutParams generateLayoutParams() {
        return new VRelativeLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
    }
...
//    }
    ...
    public static class LayoutParams extends VLayoutBase.LayoutParams {
        ...
        public final static int STR_ID_layout_toEndOf = STR_ID_VRELATIVE_LAYOUT_ATTR_BASE + 9;
        public final static int STR_ID_layout_toLeftOf = STR_ID_VRELATIVE_LAYOUT_ATTR_BASE + 10;
        public final static int STR_ID_layout_toRightOf = STR_ID_VRELATIVE_LAYOUT_ATTR_BASE + 11;
        public final static int STR_ID_layout_centerHorizontal = STR_ID_VRELATIVE_LAYOUT_ATTR_BASE + 12;
        public final static int STR_ID_layout_centerVertical = STR_ID_VRELATIVE_LAYOUT_ATTR_BASE + 13;
        public final static int STR_ID_layout_centerInparent = STR_ID_VRELATIVE_LAYOUT_ATTR_BASE + 14;
        public final static int STR_ID_VRELATIVE_LAYOUT_ATTR_COUNT = STR_ID_VRELATIVE_LAYOUT_ATTR_BASE + 15;

        @Override
        public boolean setIntValue(int key, int value) {
            boolean ret = true;
            switch (key) {
                case StringTab.STR_ID_layout_above:
                    mRules[ABOVE] = value;
                    break;
                case StringTab.STR_ID_layout_below:
                    mRules[BELOW] = value;
                    break;
                case StringTab.STR_ID_layout_alignBaseline:
                    mRules[ALIGN_BASELINE] = value;
                    break;
                case StringTab.STR_ID_layout_alignBottom:
                    mRules[ALIGN_BOTTOM] = value;
                    break;
                case StringTab.STR_ID_layout_alignEnd:
                    mRules[ALIGN_END] = value;
                    break;
                case StringTab.STR_ID_layout_alignRight:
                    mRules[ALIGN_RIGHT] = value;
                    break;
                case StringTab.STR_ID_layout_alignLeft:
                    mRules[ALIGN_LEFT] = value;
                    break;
                case StringTab.STR_ID_layout_alignStart:
                    mRules[ALIGN_START] = value;
                    break;
                case StringTab.STR_ID_layout_alignTop:
                    mRules[ALIGN_TOP] = value;
                    break;
                case StringTab.STR_ID_layout_toStartOf:
                    mRules[START_OF] = value;
                    break;
                case StringTab.STR_ID_layout_toEndOf:
                    mRules[END_OF] = value;
                    break;
                case StringTab.STR_ID_layout_toLeftOf:
                    mRules[LEFT_OF] = value;
                    break;
                case StringTab.STR_ID_layout_toRightOf:
                    mRules[RIGHT_OF] = value;
                    break;
                case StringTab.STR_ID_layout_centerHorizontal:
                    mRules[CENTER_HORIZONTAL] = value;
                    break;
                case StringTab.STR_ID_layout_centerVertical:
                    mRules[CENTER_VERTICAL] = value;
                    break;
                case StringTab.STR_ID_layout_centerInParent:
                    mRules[CENTER_IN_PARENT] = value;
                    break;
                case StringTab.STR_ID_layout_alignParentStart:
                    mRules[ALIGN_PARENT_START] = value;
                    break;
                case StringTab.STR_ID_layout_alignParentEnd:
                    mRules[ALIGN_PARENT_END] = value;
                    break;
                case StringTab.STR_ID_layout_alignParentLeft:
                    mRules[ALIGN_PARENT_LEFT] = value;
                    break;
                case StringTab.STR_ID_layout_alignParentTop:
                    mRules[ALIGN_PARENT_TOP] = value;
                    break;
                case StringTab.STR_ID_layout_alignParentRight:
                    mRules[ALIGN_PARENT_RIGHT] = value;
                    break;
                case StringTab.STR_ID_layout_alignParentBottom:
                    mRules[ALIGN_PARENT_BOTTOM] = value;
                    break;
                default:
...
//            final int id = view.getId();
         ...
         */
        void getSortedViews(IView[] sorted, int... rules) {
            final ArrayDeque<Node> roots = findRoots(rules);
            int index = 0;

            Node node;
            while ((node = roots.pollLast()) != null) {
                final IView view = node.view;
                int key = view.getVId();
        ...
        private ArrayDeque<Node> findRoots(int[] rulesFilter) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                final LayoutParams layoutParams = (LayoutParams) node.view.getVLayoutParams();
        ...
        static class Node {
            ...
            private static final Pools.SynchronizedPool<Node> sPool =
                    new Pools.SynchronizedPool<Node>(POOL_LIMIT);

            static Node acquire(IView view) {
